...
export default function ViewAmenities() {
  let [search, setSearch] = useState(false)

  let [selectAll, setSelectAll] = useState(false)
  let [ids, setIds] = useState([])

  let [amenitiesList, setAmenitiesList] = useState([])
  ...
  let getAmenitiesList = () => {
    axios.get(`${apiBaseUrl}amenities/view`,{
      params:{
        currentPage,
        title
      }
      ...
      .then((finalRes) => {
        if (finalRes.status) {
          ...
          setAmenitiesList(finalRes.data)
  ...
  let deleteAmenities = () => {
    axios.post(`${apiBaseUrl}amenities/delete`, { ids })
      .then((res) => res.data)
      .then((finalRes) => {
        if (finalRes.status) {
  ...
  let statusChange = () => {
    axios.post(`${apiBaseUrl}amenities/change-status`, { ids })
      .then((res) => res.data)
      .then((finalRes) => {
        if(finalRes.status){
          toast.success(finalRes.msg)
  ...
  let handleAll = (event) => {
    if (event.target.checked) {
      let allIds = amenitiesList.map((items) => items._id)
      setIds(allIds);
    }
    else {
      setIds([])
  ...
  useEffect(() => {
    if (amenitiesList.length >= 1) {
      if (amenitiesList.length == ids.length) {
        setSelectAll(true)
      }
      else {
        setSelectAll(false)
    ...
    <div>
      ...
      <div className={`${search ? 'max-h-[200px]' : 'max-h-[0px]'} duration-120 ease-in-out m-[20px] overflow-hidden `}>
        <div className='flex items-center gap-4 p-[20px] border-[0.1px] border-gray-300 rounded-[10px]'>
          <input onChange={(e)=>setTitle(e.target.value)} type="text" placeholder='Search Name' name="title" id="" className='px-[12px] w-[380px] h-[35px] rounded-[6px] bg-gray-700 text-gray-300 outline-[1px] outline-gray-800' />
      ...
      <div className='px-[20px] py-[20px]'>
        <div className=' border-[0.1px] border-gray-300 rounded-[8px]'>
          <div className='flex justify-between border-b-[0.1px] py-[12px] px-[20px] border-b-gray-300'>
            <h2 className='text-[22px] font-medium'>View Amenities</h2>
            <div className='flex gap-[10px]'>
              ...
              <button onClick={deleteAmenities} className='h-[40px] px-[10px] flex justify-center items-center bg-red-600 hover:bg-red-700 text-[18px] rounded-[8px] text-white cursor-pointer'>Delete</button>
            </div>
          </div>
          <div className=' rounded-[10px] bg-gray-800'>
            <div className='flex justify-between items-center bg-gray-700 p-[20px] rounded-[10px_10px_0px_0px]'>
              <div className='flex basis-[40%] gap-6 items-center'>
                ...
                <h3 className='text-gray-300 w-[35%] uppercase text-[13px] font-medium'>Title</h3>
              </div>
            ...
            {amenitiesList.length >= 1 ?
                  ...
                  <div key={index} className='flex justify-between items-center px-[20px] py-[15px]'>
                    ...
                    <div className='flex basis-[60%] justify-between  items-center text-gray-300 font-medium text-[13px]'>
                      <div className='text-left basis-[10%]'>
                        <img src={staticPath + items.image} alt="logo" width={55}  height={55} className='border-1 border-gray-700' />
                      </div>
                      <div className='text-left basis-[10%] '>
                        {items.status ?
                      ...
                      <div className='text-left basis-[10%]'>
                        <Link to={`/amenities/edit/${items._id}`}>
              ...
              <div className='px-[20px] py-[15px] text-white'> No Amenities List Found</div>
            }
